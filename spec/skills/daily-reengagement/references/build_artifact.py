...
_MNT=P.rsplit("/mnt/",1)[0]+"/mnt" if "/mnt/" in P else P
_OUT=os.path.join(_MNT,"outputs")
os.makedirs(_OUT,exist_ok=True)
data=json.load(open(f"{P}/data.json"))
tracker=json.load(open(f"{P}/tracker.json"))
DATA_JSON=json.dumps(data)
STREAK=tracker.get("streak_days",1)
DATE=data["generated"]
  ...
  <div id="home">
    <div class="top">
      <div>
        ...
        <div class="sub">__DATE__ &middot; dormant 60+ days &middot; no open deal &middot; your book (owner or ADR)</div>
      </div>
      <div class="stats">
    ...
    <div class="note">Each morning at 7:00 (weekdays) this refreshes with 3 newly-surfaced dormant accounts, fully researched with verified contacts, fresh news, and reconnection-framed A/B emails. Click a bubble to open the full run. Drafts only &mdash; you enroll.</div>
  </div>
...
function updRing(){const w=worked().length,n=DATA.companies.length;const r=document.getElementById('ring');r.style.setProperty('--p',Math.round(w/n*100));document.getElementById('ringtxt').textContent=w+"/"+n;if(w===n){completeDay();toast("All 3 worked. Streak +1. 🔥")}renderStreak()}
function grid(){
  ...
  DATA.companies.forEach((c,i)=>{
    ...
    d.innerHTML=`<div class="glow"></div><div class="donebadge">&#10003; reopened</div>
      ...
      <div class="dorm">Dormant since ${esc((c.last_touch||'').split(' ')[0])}</div></div>
...
function contact(c){
  ...
  const emailWarn=/NO EMAIL|BAD|INVALID|DUPLICATE|not found/i.test(c.email_note||"")||c.email==="not found";
  let emails="";
...
function openCo(i){
  ...
  const tiers={T1:[],T2:[],T3:[]};c.contacts.forEach(x=>tiers[x.tier].push(x));
  let body=`<button class="back" onclick="closeCo()">&larr; Back to today's 3</button>
    <div class="dhead"><div>
    ...
    ${c.reconnect_ok?`<div class="recon">&#128260; Warm reconnect &mdash; prior meeting/event on record: ${esc(c.last_touch)}</div>`:`<div class="recon" style="background:#f3f4f6;border-color:#e5e7eb;color:#374151">&#10003; Fresh outreach &mdash; dormant account, no meeting/event on record. ${esc(c.last_touch)}</div>`}
  ...
  ["T1","T2","T3"].forEach(t=>{if(tiers[t].length){body+=`<div class="tier">${t} Contacts</div>`;tiers[t].forEach(x=>body+=contact(x))}});
  const done=worked().includes(c.id);
...
"""
out=(HTML.replace("__DATA__",DATA_JSON).replace("__DATE__",DATE).replace("__STREAK__",str(STREAK)))
open(f"{P}/daily_reengagement.html","w").write(out)
open(os.path.join(_OUT,"daily_reengagement.html"),"w").write(out)
# Canonical bookmarked file (overwritten in place each morning). Same content + Cowork meta header.
META='<!doctype html><script type="application/json" id="cowork-artifact-meta">\n{\n  "name": "Daily Reengagement",\n  "schemaVersion": 1,\n  "description": "Daily Re-Engagement: 3 dormant, no-open-deal companies from Justin\'s HubSpot book. Rebuilt each weekday 7AM."\n}\n</script>\n'
index_out = out.replace('<!doctype html>', META, 1)
open(f"{P}/index.html","w").write(index_out)
open(os.path.join(_OUT,"index.html"),"w").write(index_out)
print("artifact html bytes:",len(out),"| index.html bytes:",len(index_out))
